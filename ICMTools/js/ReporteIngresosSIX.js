function CheckExcelFileReporteIngresosSIX() {
    CheckFileExists(configuraciones.carga, ValidarInformacionReporteIngresosSIX)
}


function ValidarInformacionReporteIngresosSIX(dataRequest) {
    ...
    $.ajax({
        type: "POST",
        url: "/api/reporteingresossix/validarinfo",
        ...
        error: function (xhr) {
            console.log(xhr);
            setFormStatus("error");
            $("#formatErrors").html("Error de comunicaciÃ³n (ValidarInformacionReporteIngresosSIX).");
...
function GenerarCSV(path) {
    ...
    $.ajax({
        type: "POST",
        url: "/api/reporteingresossix/insertdata",
        contentType: "application/json",
