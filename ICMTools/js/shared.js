...
function beforeUploadStarts(sender, args) {
    try {
        ...
        const newExtension = filename.substring(filename.lastIndexOf("."));

        if ((typeof pageConfig.CargaPrevia !== "undefined") || (typeof pageConfig.CargaPrevia1 !== "undefined")) {
            if (sender._hiddenField.id.includes("AsyncFileUpload1")) {
                if (sender._hiddenField.value.endsWith(".xlsx")) {
                    pageConfig.CargaPrevia = 'xlsx';
                } else if (sender._hiddenField.value.endsWith(".csv")) {
                    pageConfig.CargaPrevia = 'csv';
                } else if (sender._hiddenField.value.endsWith(".txt")) {
                    pageConfig.CargaPrevia = 'txt';
                } else {
                    pageConfig.CargaPrevia = '';
                }
            }

            if (sender._hiddenField.id.includes("AsyncFileUpload2")) {
                if (sender._hiddenField.value.endsWith(".xlsx")) {
                    pageConfig.CargaPrevia1 = 'xlsx';
                } else if (sender._hiddenField.value.endsWith(".csv")) {
                    pageConfig.CargaPrevia1 = 'csv';
                } else if (sender._hiddenField.value.endsWith(".txt")) {
                    pageConfig.CargaPrevia1 = 'txt';
                } else {
                    pageConfig.CargaPrevia1 = '';
                }
            }
            
            if (pageConfig.CargaPrevia !== 'start' && pageConfig.CargaPrevia1 !== 'start') {                
                if (pageConfig.CargaPrevia !== '' && pageConfig.CargaPrevia1 !== '') {                    
                    if (pageConfig.CargaPrevia !== pageConfig.CargaPrevia1) {
                        throw {
                            name: "Mismatched Extension",
                            message: "La extensión de los archivos cargados no coinciden.",
                            htmlMessage: "Mismatched Extension"
                        };
                        return false;
                    }
                }
        ...
        pageConfig.Extension = newExtension;
        if (ext != "xlsx" && ext != "csv") {
            ...
            throw {
                ...
                level: "Error",
                message: "Tipo de archivo inválido (Solo .xlsx y .csv).",
                htmlMessage: "Invalid File Type (Only .xlsx & .csv)."
            };
            return false;
        }
        else {
...
function uploadError(sender, args) {
    pageConfig.fileUploadSelector = "#" + sender.get_element().id;
    setFormStatus("error");
...
function ValidateExcelFile(onSuccessCallback) {
        ...
        case 'error':
            ...
            if (typeof pageConfigs !== 'undefined' && pageConfigs !== null) {
                for (let i = 0; i < selectors; i++) {
                    $(pageConfigs[i].fileUploadSelector).find('input[type=file]').val("").css("background-color", "#fff");
                }
            }
            else {
