...
function CargarInformacion(dataRequest) {
    ...
    $.ajax({
        ...
        error: function (xhr) {
            console.log(xhr);
            setFormStatus("error");
            $("#formatErrors").html("Error durante la validacion y carga de Arqueos.");
...
function EnviarInformacion(dataRequest) {
    ...
    $.ajax({
        type: "POST",
        url: "/api/arqueos/enviarinformacion",
        contentType: "application/json",
        data: JSON.stringify(dataRequest),
        success: function (response) {
            if (response.d === true) {
                ...
                $("#formatSuccess").html("Archivo de Arqueos enviado.");
            }
        },
        error: function (xhr) {
            console.log(xhr);
            ...
            $("#MensajeError").text("No se pudo completar el envio de Arqueos.");
        }
    });
}
